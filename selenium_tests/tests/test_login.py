...
def set_login_page_for_correct_credentials_test(driver: webdriver):
    driver.get("http://sd.foxtrot-titan.co.uk/app")


@when("I insert a correct username and password and submit")
def insert_correct_credentials(driver: webdriver):
    # username field
...
def check_cookie_present(driver: webdriver):
...
def check_url_is_not_login(driver: webdriver):
...
@given("the login page is displayed")
def set_login_page_for_incorrect_credentials_test(driver: webdriver):
    driver.get("http://sd.foxtrot-titan.co.uk/app")


@when("I insert an incorrect username and password and submit")
def insert_incorrect_credentials(driver: webdriver):
    # username field
...
def check_cookie_not_present(driver: webdriver):
    # we wait what we think is a timely amount of time
    sleep(1)
...
def check_url_is_login(driver: webdriver):
