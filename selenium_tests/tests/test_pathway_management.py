...
def select_pathway_to_delete(
    ...
    select = Select(index)
    select.select_by_visible_text(test_pathways[1].name)
    # driver.execute_script(
    #     # "document.getElementByName('pathwayIndex').options.forEach((opt, index) => { if (opt.text === '" + test_pathways[1].name + "' ) { document.getElementByName('pathwayIndex').selectedIndex = opt.index; }\});"
    #     "Object.values(document.getElementsByName('pathwayIndex')[0].options).forEach((option, index) => { if (option.text === '" + test_pathways[0].name + "') { document.getElementsByName('pathwayIndex')[0].selectedIndex = option.index }})"
    # )
...
#     driver.execute(
#         f"""
#             document.getElementByName('pathwayIndex').options.forEach(
#                 (opt, index) => {{
#                     if (opt.text === '{test_pathways[0].name}' ) {{
#                         document.getElementByName('pathwayIndex').selectedIndex = opt.index;
#                     }}
#                 }}
#             );
#         """
