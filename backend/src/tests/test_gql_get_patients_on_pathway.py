...
async def test_get_patients_on_pathway(context):
    ...
    assert_that(get_patients_on_pathway_result.status_code, equal_to(200)) # check the HTTP status for 200 OK
    assert_that(json.loads(get_patients_on_pathway_result.text)['data']['getPatientsOnPathway'], not_none()) # make sure there are no input errors
    patient_list=json.loads(get_patients_on_pathway_result.text)['data']['getPatientsOnPathway']
...
async def test_get_patient_on_pathway_connection(context):
    ...
    assert_that(get_patient_on_pathway_result.status_code, equal_to(200)) # check the HTTP status for 200 OK
    assert_that(json.loads(get_patient_on_pathway_result.text)['data']['getPatientOnPathwayConnection'], not_none())
    patient_list=json.loads(get_patient_on_pathway_result.text)['data']['getPatientOnPathwayConnection']


    """
    Then: we get all the patients on that pathway
    """
    assert_that(patient_list['edges'][0]['node']['id'], equal_to(str(PATIENT_ONE.id)))
    assert_that(patient_list['edges'][1]['node']['id'], equal_to(str(PATIENT_TWO.id)))
    assert_that(patient_list['edges'], not_(has_key(2)))
    ...
    assert_that(get_patient_on_pathway_result_cursor.status_code, equal_to(200)) # check the HTTP status for 200 OK
    assert_that(json.loads(get_patient_on_pathway_result_cursor.text)['data']['getPatientOnPathwayConnection'], not_none())
    patient_list_cursor=json.loads(get_patient_on_pathway_result_cursor.text)['data']['getPatientOnPathwayConnection']


    """
    Then: we get all the patients on that pathway
    """
    assert_that(patient_list_cursor['edges'][0]['node']['id'], equal_to(str(PATIENT_THREE.id)))
    assert_that(patient_list_cursor['edges'], not_(has_key(1)))