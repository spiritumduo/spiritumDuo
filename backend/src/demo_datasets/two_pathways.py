...
async def insert_two_pathways_data(SESSION_COOKIE: str = None):
    ...
    lung_cancer_clinical_request_types: List[ClinicalRequestType] = [
        ...
        await ClinicalRequestType.create(
            ...
            ref_name=(
                "Biopsy of thorax using computed tomography "
                "guidance (procedure)"
            ),
            is_test_request=True
        ),
    ...
    lymphoma_clinical_request_types: List[ClinicalRequestType] = [
        await ClinicalRequestType.create(
            name="I don't know",
    ...
    for pathwaySetIndex in range(1, NUMBER_OF_PATHWAYS + 1):
        ...
        for mT in lymphoma_clinical_request_types:
            await PathwayClinicalRequestType.create(
                pathway_id=lymphoma_pathway.id,
                clinical_request_type_id=mT.id
            )
