...
#  PageInfo
def has_previous_page(all_edges=None, before=None, after=None, first=None, last=None):
    if last is not None:
        edges = apply_cursors_to_edges(all_edges, before, after)
        if len(edges) > last:
            return True
...
def has_next_page(all_edges=None, before=None, after=None, first=None, last=None):
    if first is not None:
        edges = apply_cursors_to_edges(all_edges, before, after)
        if len(edges) > first:
            return True
...
def make_connection(models, before, after, first, last):
    ...
    page_info = {
        "has_previous_page": has_previous_page(all_edges, before, after, first, last),
