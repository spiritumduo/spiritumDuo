...
async def resolve_get_patients_on_pathway(obj=None, info=None, id=None, filter=None, discharged=False):
    ...
    if filter and discharged:
        ptPwInstances=await db_sync_to_async(list)(PatientPathwayInstance.objects.filter(pathway=id, awaiting_decision_type=filter, is_discharged=discharged))
    elif filter is not None:
        ptPwInstances=await db_sync_to_async(list)(PatientPathwayInstance.objects.filter(pathway=id, awaiting_decision_type=filter))
    elif discharged is not None:
        ptPwInstances=await db_sync_to_async(list)(PatientPathwayInstance.objects.filter(pathway=id, is_discharged=discharged))
    ptList=[]
