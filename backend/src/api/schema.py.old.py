...
class RoleType(InputObjectType):
    class Meta:
        model = Role
        fields = ("id", "name")
...
class CreateDecisionPoint(graphene.Mutation):
    ...
    class Arguments:
        id=graphene.ID()
        patient=graphene.Scalar(PatientType)
        addedAt=graphene.Date()
        updatedAt=graphene.Date()
        clinician=graphene.Scalar(UserType)
        decisionType = DecisionPointTypes()
...
class CreateTestResult(graphene.Mutation):
    ...
    def mutate(self, info, id, patient, addedAt, description, mediaUrls):
        ...
        newTestResult.save()
        return CreateTestResult(testResult=newTestResult)
