...
class RequestPlaceholder(dict):
    """
    This is a class that has the properties of
    a dictionary too. This is to emulate how
    Starlette's request class operates, as context
    has to be passed to some functions to get
    the db connection and user objects
    """
        ...
        for on_pathway in on_pathways:
            ...
            if on_pathway_counter == 1 or (
                ...
                """
                    ...
                    want to add any decision points as they will need to be
                    triaged
                """
                if not is_patient_new:
                    ...
                    possible_milestone_types = [
                        selectable_milestone_types[
                            randint(0, len(selectable_milestone_types)-1)
                        ],
                        selectable_milestone_types[
                            randint(0, len(selectable_milestone_types)-1)
                        ]
                    ...
                    if has_clinic:
                        await CreateDecisionPoint(
                            ...
                            milestone_requests=[
                                {"milestoneTypeId": 
                                selectable_milestone_types[
                                    randint(0, len(selectable_milestone_types)-1)].id
                                }
                            ]
                        )
                        await on_pathway.update(
